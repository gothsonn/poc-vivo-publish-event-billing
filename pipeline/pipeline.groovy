node('docker-node') {
  ...
  stage('Deploy Image'){
    ...
    withCredentials([file(credentialsId: 'kubeconfig', variable: 'SECRET_FILE')]) {
      ...
      sh 'KUBECONFIG=$SECRET_FILE kubectl set image deployment/publish-event-billing publish-event-billing=registry.devops.7f8254f4188647b4be19.eastus.aksapp.io/'+fullname
    }
  }
